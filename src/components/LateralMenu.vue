	...
	<div id='lateral_menu'>
		...
		<div id="types_search">
			<div id='types_search_title'>
				<FilterButton class="flex_item" @click.native="$emit('exclusiveFilterToggle');" />
				<p class="flex_item" v-on:click="showTypesSearch = !showTypesSearch;"> Types </p>
			</div>
	
			<div id='types_search_btns' v-show="showTypesSearch">
				<Type v-for="type in types" :isLocked="false" :key="type" :name='type' :id='type' class='type_btn' :disabled="true" :ref="type" @click.native="changeFilter($event.target)" />
			</div>
